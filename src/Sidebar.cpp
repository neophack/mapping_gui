...
void panorama::Sidebar::renderUI() {
    ...
    // CPU Pane button
    if (ImGui::Selectable(ICON_FA_MICROCHIP " Mapping",
                          (m_eCurrentlyVisiblePane == PaneType::PANETYPE_CPU), 0, v2ButtonSize))
        m_eCurrentlyVisiblePane = PaneType::PANETYPE_CPU;

    // Processes Pane button
    if (ImGui::Selectable(ICON_FA_WINDOW_RESTORE " Loop Closing",
    ...
    if (ImGui::Selectable("About...")) {
        bIsAboutOpen = true;
        ImGui::OpenPopup("About Mapping GUI...");
    }
    ...
    // Render about popup?
    if (ImGui::BeginPopupModal("About Mapping GUI...", &bIsAboutOpen, ImGuiWindowFlags_NoResize)) {
